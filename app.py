...
def projectMemberCancel(prj_id):
    ...
    if ProjectMember_info:
        try:
            ...
            if not unConfirm: 
                Project_info = Project.query.finter_by(prj_id = prj_id) .first()
                Project_info.prj_status ='020'
                db.session.add(Project_info)
                db.session.commit()
...
def getMember():
    ...
    memberItem=Member.query.all()
    return jsonify([*map(member_serializer,memberItem)])
    #return jsonify(member_serializer(memberItem)), 200
    #memberItem=Member.query.get_or_404(
    #    get_jwt_identity(), description=expected)
    #return jsonify(member_serializer(memberItem)), 200
